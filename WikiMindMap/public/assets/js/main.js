    ...
    methods: {
        async search(params) {
            ...
            if(matched && matched[0]) {
                ...
                if(this.activeMenu?.file && this.activeMenu.file.endsWith('.csv')) {
                    // V1: csv file
                    d3.csv(this.activeMenu.file, function(d) {return { ...d };}).then((data) => {
                        // if (error) throw error;
                        adata = data;
                        adatajson = convertArrayToJson(adata);
                ...
                } else if(this.activeMenu?.file && this.activeMenu.file.endsWith('.json')) {
                    // V2: Json file
                    var data = await d3.json(this.activeMenu.file, function(d) {return { ...d };})
                    
                    const root = d3.hierarchy(data);
                    tree(root);

                    var dataArray = convertJsonToArray(data);
                    this.init(dataArray);
        ...
        init(data) {
            ...
            for (let i = 0; i < data.length; i++) {
                let {id,href,value,name,type,content,related,tags,excerpt, hasdetailedpage, shortdescription, description, expanded} = data[i];
                ...
                if (href && name && (content || description)) {
         ...
         * @param {*} param0 
         */
        getContentTemplate({id,href,value,name,type,content,related,tags,excerpt,hasdetailedpage, shortdescription, description, expanded}) {
            if(!name) {
                name = id?.substring(id.lastIndexOf(".") + 1)
            }

            var itemContent = content || description;
            if (!href || !itemContent) {
                return '';
            }
...
function convertArrayToJson(csvData) {
    if(!csvData) {
        return {};
    }
    var result = {};
...
function convertJsonToArray(jsonItem) {
    var results = [];
    if(!jsonItem) {
        return [];
    }

    // {id,href,value,name,type,content,related,tags,excerpt} 
    if(jsonItem?.id || jsonItem?.name || jsonItem?.href || jsonItem?.description) {
        results.push({
            id: jsonItem.id,
            href: jsonItem.href,
            name: jsonItem.name,
            ...
            expanded: jsonItem.expanded
        });
    }

    if(jsonItem.children) {
        for(var i=0; i<jsonItem.children.length; i++) {
            var child = jsonItem.children[i];
            results.push.apply(results, convertJsonToArray(child));
        }
    }
